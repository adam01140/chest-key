...
for (let x = 0; x < dung.width; x++) {
    for (let y = 0; y < dung.height; y++) {
        if (dung.matrix[x][y] !== null) {
            let roomCoord = [x * roomWidth, (dung.height - 1 - y) * roomHeight];
            let chest = this.physics.add.sprite(roomCoord[0] + roomWidth / 10, roomCoord[1] + roomHeight / 35, 'chest');
            
            // Set up initial immovable state for the chest
            chest.body.immovable = true; // Ensures the chest does not move on collision
            chest.body.moves = false;  // Prevents the chest from being displaced by physics forces



// Set up a collider with conditional logic
this.physics.add.collider(this.hero, chest, function(hero, chest) {
    ...
    // This function returns true when the collision should occur
    
	if(has_keys == 1){
	chest.disableBody(true, true); // Disable and hide the chest
    has_keys -= 1;
	}
	//return has_keys <= 0;  // Only collide (block the player) if no keys are left
}, this);
    ...
    for (let y = 0; y < dung.height; y++) {
        if (dung.matrix[x][y] !== null) {
            ...
            // Assuming keys are small enough and do not need to be offset further than center.
            let key = this.physics.add.sprite(roomCoord[0] + roomWidth / 105, roomCoord[1] + roomHeight / 35, 'key');
            
            // Collision detection for key pickup
            this.physics.add.overlap(this.hero, key, function(hero, key) {
                key.disableBody(true, true); // Disable and hide the key
				has_keys = 1;
				alert("You got " + has_keys + " keys");
            }, null, this);
        }
    }
}
